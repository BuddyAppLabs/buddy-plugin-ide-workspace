...
const logger = new Logger('KiroService');

/**
 * Kiro工作空间服务
 */
export class KiroService extends BaseIDEService {
    /**
     * 获取Kiro的工作空间路径
     */
    async getWorkspace(): Promise<string | null> {
        try {
            ...
            if (!storagePath) {
                logger.error('未找到Kiro存储文件');
                return null;
            }

            // 读取并解析存储文件
            const content = fs.readFileSync(storagePath, 'utf8');
            const workspacePath = this.parseKiroJson(content);
            ...
            logger.error('获取Kiro工作空间失败:', error);
            return null;
        }
    }
    ...
    async findStoragePath(): Promise<string | null> {
        ...
        if (process.platform === 'darwin') {
            possiblePaths = [
                path.join(home, 'Library/Application Support/Kiro/storage.json'),
                path.join(
                    home,
            ...
            if (appData) {
                possiblePaths = [
                    path.join(appData, 'Kiro/storage.json'),
                    path.join(appData, 'Kiro/User/globalStorage/storage.json'),
                ];
            }
        } else if (process.platform === 'linux') {
            possiblePaths = [
                path.join(home, '.config/Kiro/storage.json'),
                path.join(home, '.config/Kiro/User/globalStorage/storage.json'),
            ];
        }
        ...
        for (const filePath of possiblePaths) {
            if (fs.existsSync(filePath)) {
                logger.debug(`找到Kiro存储文件: ${filePath}`);
                return filePath;
            }
        }
     ...
     * 解析Kiro JSON格式的存储文件
     */
    private parseKiroJson(content: string): string | null {
        try {
            ...
            if (windowState?.lastActiveWindow?.folder) {
                ...
                logger.info(`找到Kiro工作区: ${workspacePath}`);
