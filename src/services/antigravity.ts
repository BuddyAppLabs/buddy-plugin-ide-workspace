...
const logger = new Logger('AntigravityService');

/**
 * Antigravity工作空间服务
 */
export class AntigravityService extends BaseIDEService {
    /**
     * 获取Antigravity的工作空间路径
     */
    async getWorkspace(): Promise<string | null> {
        try {
            const storagePath = await this.findStoragePath();
            if (!storagePath) {
                logger.error('未找到Antigravity存储文件');
                return null;
            }

            // 读取并解析存储文件
            const content = fs.readFileSync(storagePath, 'utf8');
            const workspacePath = this.parseAntigravityJson(content);
            ...
            logger.error('获取Antigravity工作空间失败:', error);
            return null;
        }
    }
    ...
    async findStoragePath(): Promise<string | null> {
        ...
        if (process.platform === 'darwin') {
            possiblePaths = [
                path.join(home, 'Library/Application Support/Antigravity/storage.json'),
                path.join(
                    home,
            ...
            if (appData) {
                possiblePaths = [
                    path.join(appData, 'Antigravity/storage.json'),
                    path.join(appData, 'Antigravity/User/globalStorage/storage.json'),
                ];
            }
        } else if (process.platform === 'linux') {
            possiblePaths = [
                path.join(home, '.config/Antigravity/storage.json'),
                path.join(home, '.config/Antigravity/User/globalStorage/storage.json'),
            ];
        }
        ...
        for (const filePath of possiblePaths) {
            if (fs.existsSync(filePath)) {
                logger.debug(`找到Antigravity存储文件: ${filePath}`);
                return filePath;
            }
        }
    ...
    /**
     * 解析Antigravity JSON格式的存储文件
     */
    private parseAntigravityJson(content: string): string | null {
        try {
            ...
            if (windowState?.lastActiveWindow?.folder) {
                ...
                logger.info(`找到Antigravity工作区: ${workspacePath}`);
