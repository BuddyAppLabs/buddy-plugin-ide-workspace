 ...
 * Qoder工作空间服务
 */
export class QoderService extends BaseIDEService {
    /**
     * 获取Qoder的工作空间路径
     */
    async getWorkspace(): Promise<string | null> {
        try {
            ...
            if (storagePath.endsWith('.json')) {
                ...
                // 在 qoder 目录下查找文件信息
                workspacePath = await this.findWorkspaceInQoderFolder();
            }
            ...
            logger.error('获取Qoder工作空间失败:', error);
            return null;
        }
    }
    ...
    async findStoragePath(): Promise<string | null> {
        ...
        if (process.platform === 'darwin') {
            possiblePaths = [
                path.join(home, 'Library/Application Support/Qoder/storage.json'),
                path.join(
                    home,
                ...
                path.join(
                    home,
                    'Library/Application Support/Qoder/User/globalStorage/storage.json'
                ),
                path.join(
                    home,
                    'Library/Application Support/Qoder - Insiders/storage.json'
                ),
                path.join(
                    home,
                    'Library/Application Support/Qoder - Insiders/User/globalStorage/state.vscdb'
                ),
                path.join(
                    home,
                    'Library/Application Support/Qoder - Insiders/User/globalStorage/storage.json'
                ),
            ];
            ...
            possiblePaths = [
                ...
                path.join(home, '.config/Qoder/User/globalStorage/state.vscdb'),
        ...
        for (const filePath of possiblePaths) {
            if (fs.existsSync(filePath)) {
                logger.debug(`找到Qoder存储文件: ${filePath}`);
                return filePath;
            }
        }
     ...
     */
    private async findWorkspaceInQoderFolder(): Promise<string | null> {
        try {
            const home = os.homedir();
            let workspaceFolders: string[] = [];

            // 根据操作系统确定Qoder工作区文件夹可能的位置
            if (process.platform === 'darwin') {
                workspaceFolders = [
                    path.join(home, 'Library/Application Support/Qoder/User/workspaceStorage'),
                    path.join(home, 'Library/Application Support/Qoder - Insiders/User/workspaceStorage')
                ];
            } else if (process.platform === 'win32') {
                const appData = process.env.APPDATA;
                if (appData) {
                    workspaceFolders = [
                        path.join(appData, 'Qoder/User/workspaceStorage')
                    ];
                }
            } else if (process.platform === 'linux') {
                workspaceFolders = [
                        ...
                        try {
                            ...
                            if (data.folder) {
                                ...
                                if (fs.existsSync(workspacePath)) {
                                    logger.debug(`[QoderService] Found workspace from workspace.json: ${workspacePath}`);
            ...
            if (recentFolders.length > 0) {
                logger.debug(`[QoderService] Using recent folder as workspace: ${recentFolders[0]}`);
